...
pub fn annotate(garden: &[&str]) -> Vec<String> {
    ...
    let chars: Vec<u8> = garden.iter().flat_map(|row| row.bytes()).collect();
    ...
    for (index, char) in chars.iter().enumerate() {
        ...
        let ns = neighbor_indices(index, width, total_indices);
        let n_count: u8 = ns.iter().filter(|&&n| chars[n] == b'*').count() as u8;
        if n_count > 0 {
           dbg!(stars.push( (b'0' + n_count)));
        } else {
            stars.push(b' ');
        }
    }
