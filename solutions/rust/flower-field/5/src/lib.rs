...
pub fn neighbor_indices(row: usize, col: usize, width: usize, rows: usize) -> Vec<(usize,usize)> {
    let index_diffs = [
        (0, 1), (0, -1), // right, left
        (-1, 0), (1, 0), // up, down
...
pub fn annotate(garden: &[&str]) -> Vec<String> {
    ...
    let chars: Vec<Vec<u8>> = garden.iter().map(|row| row.bytes().collect()).collect();
    let total_indices = chars.len();
    let mut stars: Vec<u8> = vec![];
    for (row, cols) in chars.iter().enumerate() {
        for (col, c) in cols.iter().enumerate() {
            if *c == b'*' {
                stars.push(b'*');
                continue;
            }
            let ns = neighbor_indices(row, col, width, garden.len());
            let n_count: u8 = ns.iter().filter(|&&(row, col)| chars[row][col] == b'*').count() as u8;
            if n_count > 0 {
                stars.push( b'0' + n_count);
            } else {
                stars.push(b' ');
            }
        }
    ...
    stars
        .chunks(width)
        .map(|chunk| String::from_utf8(chunk.to_vec()).expect("invalid utf-8"))
        .collect()
